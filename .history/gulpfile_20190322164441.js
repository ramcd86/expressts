...
gulp.task('ts-server-controllers', function() {
    return gulp.src(['./src/server/controllers/*.ts'])
        .pipe(tsc({
            typescript: require('typescript')
        }))
        .pipe(gulp.dest('./public_html/server/controllers'));
})
