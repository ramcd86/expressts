...
gulp.task('compile-es6', function() {
    return gulp.src(
        ...
        .pipe(gulp.dest('./public_html/server/es6output'))
})

gulp.task('concat-client', function() {
    return gulp.src('./public_html/server/es6output/**/*.js')
    .pipe(sourcemaps.init())
    .pipe(concat('main.js'))
    .pipe(sourcemaps.write())
    .pipe(gulp.dest('./public_html/server/client'));
...
gulp.task('restructure-data', function() {
    ...
    gulp.src(['./public_html/client-preconc/main.css'])
    .pipe(gulp.dest('./public_html/server/client'))
})
